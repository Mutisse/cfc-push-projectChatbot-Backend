...
export class WhatsAppController {
  ...
  async sendWelcomeMessage(req: Request, res: Response) {
    try {
      ...
      res.json({
        success: result,
        message: result ? 'Mensagem de boas-vindas enviada' : 'Erro ao enviar mensagem',
        data: { phone, userName }
      });
    } catch (error) {
      res.status(500).json({
  ...
  async sendPrayerConfirmation(req: Request, res: Response) {
    try {
      ...
      const result = await whatsappService.sendPrayerConfirmation(phone, prayerSubject);
      
      res.json({
        success: result,
        message: result ? 'Confirmação de oração enviada' : 'Erro ao enviar confirmação',
  ...
  async getProviderStatus(req: Request, res: Response) {
    try {
      // Este método seria implementado no provider real
      const status = {
        configured: false,
        service: 'WhatsApp',
