...
const config = {
  ...
  CHATBOT_URL:
    ...
    "https://cfc-push-projectchatbot-backend.onrender.com",
  ...
  CORS_ORIGIN:
    ...
    "http://localhost:9000,http://localhost:8080,http://localhost:3000",
  ...
  ALERT_THRESHOLD_RESPONSE_TIME: parseInt(
    process.env.ALERT_THRESHOLD_RESPONSE_TIME || "500"
  ),
  ALERT_THRESHOLD_ERROR_RATE: parseInt(
    process.env.ALERT_THRESHOLD_ERROR_RATE || "5"
  ),
...
if (config.NODE_ENV === "development") {
  ...
  console.log(
    `MONGO_URI: ${config.MONGO.URI ? "✅ Configurado" : "❌ Não configurado"}`
  );
  console.log(`DB_NAME: ${config.MONGO.DB_NAME}`);
  console.log(
    `REDIS: ${config.REDIS_ENABLED ? "✅ Habilitado" : "❌ Desabilitado"}`
  );
  console.log("=============================");
}
